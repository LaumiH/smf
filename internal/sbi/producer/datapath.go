...
// Sends PDRs of all datapaths to UPFs and sends the status to the UE.
...
func RestorePDUSessionAtUPF(
	...
	logger.PduSessLog.Infof("Init session restoration on UPF[%s] for %s", nodeID, pfcpSessionContext.PDUSessionParams())
...
func modifyExistingPfcpSession(
	...
	select {
	case <-upf.Association.Done():
		logger.PduSessLog.Warnf("UPF[%s] no longer associated, do not modify %s", nodeID, pfcpSessionContext.PDUSessionParams())
		resCh <- smf_context.SendPfcpResult{
	...
	if rsp.Cause != nil && rsp.Cause.CauseValue == pfcpType.CauseRequestAccepted {
		...
		logger.PduSessLog.Errorf("Received SessionModificationReject for %s from UPF[%s]", pfcpSessionContext.PDUSessionParams(), nodeID)
		resCh <- smf_context.SendPfcpResult{
			Status: smf_context.SessionUpdateFailed,
			Err:    fmt.Errorf("cause[%d] if not request accepted", rsp.Cause.CauseValue),
			RcvMsg: rcvMsg,
...
func restorePfcpSession(
	...
	select {
	case <-upf.Association.Done():
		logger.PduSessLog.Warnf("UPF[%s] not associated, do not restore %s", nodeID, pfcpSessionContext.PDUSessionParams())
		resCh <- smf_context.SendPfcpResult{
			Status: smf_context.SessionEstablishFailed,
			Source: nodeID,
		}
...
func releasePfcpSession(
	...
	select {
	case <-upf.Association.Done():
		logger.PduSessLog.Warnf("UPF[%s] not associated, do not release %s", nodeID, pfcpSessionContext.PDUSessionParams())
		resCh <- smf_context.SendPfcpResult{
	...
	logger.PduSessLog.Infof("Sending SessionReleaseRequest for %s to UPF[%s]", pfcpSessionContext.PDUSessionParams(), nodeID)
...
func waitAllPfcpRsp(
	...
	for i := 0; i < pfcpPoolLen; i++ {
		...
		// UPF can be disassociated at this point due to delayed heartbeat, which detected a false-positive failure
		...
		if res.Status == smf_context.SessionEstablishFailed ||
			...
			logger.PfcpLog.Errorf("Session management process failed due to at least one UPF reporting %s", res.Status)
